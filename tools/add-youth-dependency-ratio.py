#!/usr/bin/env python3
"""Add annual youth dependency ratios to country-demographic-metrics.json.

The ratio is children ages 0-14 per 100 working-age adults ages 15-64:

  youthDependencyRatio = population(0-14) / population(15-64) * 100
...
def ratio_grid_for_country(
    ...
    for year_index in range(year_count):
        youth_total = total_at(male, year_index, age_count, youth_indices)
        youth_total += total_at(female, year_index, age_count, youth_indices)
        ...
        ratios.append(youth_total / working_total * 100 if working_total else None)
    return ratios
...
def main() -> None:
    ...
    youth_indices = group_indices(age_groups, YOUTH_AGE_GROUPS)
    ...
    for iso3, metrics in metrics_payload["countries"].items():
        ...
        if any(value is None for value in series):
            missing.append(iso3)
        metrics["youthDependencyRatio"] = series

    OUTPUT.write_text(json.dumps(metrics_payload, separators=(",", ":")))

    if missing:
        raise SystemExit(f"Missing youth dependency ratios for: {', '.join(missing)}")
    print(
        f"Added annual youth dependency ratios for "
